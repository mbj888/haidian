...
2. **社区健康站**：用户=居民（含老幼）；空间=AI 原点社区；数据=体征（知情同意）；AI=健康风险提示；运营=社区卫生中心；复核=医师日审；隐私=最小化采集、加密；回退=转线下门诊；指标=覆盖人数；停止=误报率>5%（本方案试点讨论预设阈值，尚未经真实临床数据验证；不构成行业通行标准或专业验收依据）[assumption:scenario_threshold_unverified]。
...
11. **AI 文保巡检员**：用户=文保/运维人员；空间=铁路遗址公园；数据=轨道/建筑图像与振动传感器；AI=病害/异常识别；运营=文旅运营方；复核=文保专家月审；隐私=不采集人脸；回退=人工巡检；指标=病害检出率；停止=误判率>10%（本方案试点讨论预设阈值，尚未经文保专家标注数据集校准；不构成行业标准或专业验收依据）[assumption:scenario_threshold_unverified]。
...
### 可实施性量化参数（provisional 估算框架）

为回应任务书"可实施性"维度，以下参数基于临时几何与公告面积给出**概念级估算框架**；精确投资金额待项目级工程概算确定；所有数值在官方红线、权属、市政容量与工程条件到达后必重算，不得替代正式投资决策 [assumption:implementation_provisional][assumption:cost_benchmark_method_only]。

| 参数 | 近期（众智园） | 中期（AI 原点社区） | 远期（大钟寺+全域） | 估算依据 |
|---|---|---|---|---|
| 重点区面积 | 192.1 ha | 104.3 ha | 72.0 ha | 官方公告值 [source:haidian_gov_2026] |
| 情景测算建筑面积（按 FAR 1.0 设计假设） | 约 192 万 m² | 约 104 万 m² | 约 72 万 m² | 面积×FAR 1.0（设计假设目标，非审定控制或开发许可结论）[metric:floor_area_ratio][assumption:far_scenario] |
| 概念更新比例（活化+轻改造） | 15% | 20% | 25% | 由遗址保护密度递减假设 [assumption:implementation_provisional] |
| 概念更新建筑面积 | 约 29 万 m² | 约 21 万 m² | 约 18 万 m² | 上两项乘积 |
| 概念投资框架（待项目级工程概算确定） | 待正式造价咨询 | 待正式造价咨询 | 待正式造价咨询 | 成本科目：土建/机电/装修/文保加固/市政接驳；方法：参考同类城市更新项目公开区间，须以正式造价咨询报告与审批为准 [assumption:cost_benchmark_method_only] |
| 文保/遗产保留率目标 | ≥95% | ≥90% | ≥85% | 遗址公园与工业遗产原真性优先 [assumption:implementation_provisional] |
| 公共空间容量（按 5% 公共空间比） | 约 9.6 ha | 约 5.2 ha | 约 3.6 ha | 面积×public_space_ratio 5% [metric:public_space_ratio] |
| 入驻承诺验证机制 | 新建/重大改造载体须取得 ≥60% 意向入驻或 LOI | 社区级设施须与运营主体签订服务协议 | 商业更新须取得主力租户 MOU | 避免"场景轻产品"的硬性门槛 [depth:implementation_logic] |
| 失败回退判定 | 无中试落地或意向入驻率<40% 时暂缓扩建 | 微循环准点率<80% 或运营协议未达成时缩至单点 | 主力租户 MOU<30% 或现金流预测为负时保留存量 | 分期验证逻辑 [depth:renewal_project_list] |

> 说明：上表所有金额、比例、阈值均为**概念假设**，用于论证方案具备可深化的财务—空间框架；正式投资须以法定规划条件、工程概算、产权谈判与审批结果为准 [assumption:implementation_provisional]。
...
| floor_area_ratio (FAR) | 1.0 | — | assumption/low | 设计假设目标值，非审定容积率或控制指标；须以官方控规条件重算 [assumption:far_scenario] |
...
为强化任务书"AI 治理全球话语权"功能，本方案不将"治理"简化为合规清单，而是把设计过程本身变成可复用的开放治理基础设施 [depth:ai_governance]。

- **机器可读任务书与开源评审**：将任务书、指标、假设、来源、合规矩阵以结构化 JSON/Markdown 公开，使外部研究者、监管方与市民可逐条追踪设计决策；本次提交即采用该范式 [standard:data_governance]。
- **多方治理理事会**：建议设立由政府、高校、企业、居民代表、文保专家与算法伦理委员组成的"京张 AI 城市治理理事会"，对 12 类场景卡的算法更新、数据使用、停止条件进行季度审议；理事会决议与异议记录同步公开 [depth:ai_governance]。
- **算法审计与伦理沙盒**：产业合规沙盒（场景卡 10）与无人微循环、健康、助老等高敏场景须通过第三方算法影响评估，评估模板开源；任何场景上线前须公布训练数据来源、性能基准、误报/误判率阈值与人工接管协议 [standard:human_oversight][assumption:scenario_threshold_provisional]。
- **跨境知识输出**：以"Jingzhang Origin Axis"双语品牌与 GitHub 名刻碑机制，把本带的场景实测、失败回退、治理决议沉淀为可翻译、可引用的开放知识包；目标是为全球工业遗产城市更新与 AI 城市治理提供一套可验证的东方样本，而非单向输出技术 [depth:branding_loop][depth:long_term_ops]。
