...
2. **社区健康站**：用户=居民（含老幼）；空间=AI 原点社区；数据=体征（知情同意）；AI=健康风险提示；运营=社区卫生中心；复核=医师日审；隐私=最小化采集、加密；回退=转线下门诊；指标=覆盖人数；停止=误报率>5%（provisional：基层医疗 AI 常见可接受上限，待真实临床数据验证后调整）[assumption:scenario_threshold_provisional]。
...
11. **AI 文保巡检员**：用户=文保/运维人员；空间=铁路遗址公园；数据=轨道/建筑图像与振动传感器；AI=病害/异常识别；运营=文旅运营方；复核=文保专家月审；隐私=不采集人脸；回退=人工巡检；指标=病害检出率；停止=误判率>10%（provisional：文保辅助识别优先保证不漏检，允许较高误检率；待专家标注数据集校准后调整）[assumption:scenario_threshold_provisional]。
...
为回应任务书"可实施性"维度，以下参数基于临时几何、公告面积与公开成本区间给出**概念级估算**，仅供专业团队深化参考；所有数值在官方红线、权属、市政容量与工程条件到达后必重算，不得替代正式投资决策 [assumption:implementation_provisional][assumption:cost_benchmark_public]。

| 参数 | 近期（众智园） | 中期（AI 原点社区） | 远期（大钟寺+全域） | 估算依据 |
|---|---|---|---|---|
| 重点区面积 | 192.1 ha | 104.3 ha | 72.0 ha | 官方公告值 [source:haidian_gov_2026] |
| 按 FAR 1.0 计可建/更新总建筑面积 | 约 192 万 m² | 约 104 万 m² | 约 72 万 m² | 面积×FAR 1.0 [metric:floor_area_ratio] |
| 概念更新比例（活化+轻改造） | 15% | 20% | 25% | 由遗址保护密度递减假设 [assumption:implementation_provisional] |
| 概念更新建筑面积 | 约 29 万 m² | 约 21 万 m² | 约 18 万 m² | 上两项乘积 |
| 概念投资区间 | 15–45 亿元 | 10–35 亿元 | 8–30 亿元 | 公开存量更新成本 5,000–15,000 元/m² 区间估算 [assumption:cost_benchmark_public] |
