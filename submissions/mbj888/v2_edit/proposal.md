...
2. **社区健康站**：用户=居民（含老幼）；空间=AI 原点社区；数据=体征（知情同意）；AI=健康风险提示；运营=社区卫生中心；复核=医师日审；隐私=最小化采集、加密；回退=转线下门诊；指标=覆盖人数；停止=误报率>试点预设阈值(待实测标定)。
...
11. **AI 文保巡检员**：用户=文保/运维人员；空间=铁路遗址公园；数据=轨道/建筑图像与振动传感器；AI=病害/异常识别；运营=文旅运营方；复核=文保专家月审；隐私=不采集人脸；回退=人工巡检；指标=病害检出率；停止=误判率>试点预设阈值(待实测标定)。
...
**成本与资金口径（数据缺口声明）**：分期中的投资/造价均为概念性规划假设，无定价来源、无资金承诺；不用于正式预算或招商要约，官方红线与控规下达后以公告口径重算 [assumption:cost_provisional][data:gap/cost_unsourced]。
...
本方案已建立近期—中期—远期阶段路径、三处先行单元、运营主体、进入下一阶段判定与失败回退，并通过 [data:geometry/phasing.geojson]、[metric:phasing_stage]、更新项目清单与场景责任方形成可移交框架。为避免把未核验内容写成结论，以下六项现状与成本基线**全部登记为待核验（unknown）**，并在 metrics.json 与 assumptions.json 中给出可核验路径。

| 基线项 | 当前状态 | 核验路径 | 责任方 | 触发条件 |
|---|---|---|---|---|
| 现状交通 [metric:traffic_baseline_verified] | 未知：无交通量、路段通行能力、信号配时与公共交通供给数据 | 以官方或实测路网替换示意路网，按分车型计数与信号周期计算通行能力与服务水平 | participant 计算 / organizer 或交管部门提供数据 | 进入工程设计、交通影响评价或实施论证前 |
| 步行可达性 [metric:walkability_baseline_verified] | 未知：无人行道、过街设施、阻隔断点与步行流量数据 | 构建真实步行拓扑，按 500m/800m 步行距离计算公共服务与公共空间覆盖率，并标注铁路与快速路阻隔点 | participant 计算 / organizer 或权属部门提供数据 | 慢行系统深化、无障碍设计或站点周边详细设计启动前 |
| 产业服务 [metric:industry_service_baseline] | 未知：无产业用地台账、建筑存量、入驻结构与公共服务容量数据 | 以产业用地与建筑普查台账建立存量清单，按可租售面积与配套容量核算承载上限，并与公共服务设施服务半径叠加校核 | participant 核算 / organizer 或产业主管部门提供数据 | 招商策划、产业导入政策设计或运营规模论证前 |
| 土地与建筑权属 [metric:land_ownership_verified] | 未知：无权属、建筑安全鉴定、文保评估与地下条件资料 | 按宗地核对权属与用途，开展建筑安全与文化价值鉴定，补做地下条件与管线探测，据此调整留改拆建与分期 | shared（权属部门、鉴定机构与 participant） | 任何片区进入实施、征收、改造或工程设计前 |
| 市政容量 [metric:municipal_capacity_baseline] | 未知：无现状管网口径、剩余容量，亦无按规范测算所需的建筑面积与人口基数 | 以审定建筑面积与人口规模为基数，按现行给水、排水、电力、通信与燃气规划规范测算负荷，与现状管网剩余容量比对并提出增容方案 | participant 测算 / 市政权属单位提供容量与口径 | 市政专项规划、分期开发强度确定或任一地块开工前 |
| 成本基线 [metric:cost_baseline_verified] | 未知，且为**红线项**：按 README L233，投资测算不得写成已确定结论 | 以北京市现行建设工程造价指标与同类城市更新项目结算数据为单价依据，按分期的建筑、市政、景观与征拆工程量测算，并明确资金来源与主体 | participant 测算 / 造价咨询机构与资金主体提供依据 | 进入预算编制、投资决策或融资安排前；此前成本仅作概念性框架登记 |

**边界声明**：上表六项在本包内均不提供数值，也不作为预算、审批、工程或招商依据。包内 roads.geojson 仅 3 条 agent 推断中心线（source_type=agent_inferred_from_public_data, confidence=medium），仅用于示意，不得用于交通量或步行可达性计算。铁路与快速路造成的功能割裂登记为设计假设（[assumption:urban_fragmentation_hypothesis]），须经现场调查或经审查基线核验后方可作为事实使用。组织方几何与数据缺口依 README 不阻断内容评分，本方案亦未因该缺口而省略上述披露。
