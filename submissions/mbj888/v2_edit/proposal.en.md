...
- Land-use partition and FAR in `geometry/land_use.geojson`; overall FAR is a design-scenario assumption (~1.0) registered under assumption:far_assumed; it is a planning reference value, not an approved control indicator, and is declared status=unknown in metrics.json [data:geometry/land_use.geojson][metric:floor_area_ratio][assumption:far_assumed].
